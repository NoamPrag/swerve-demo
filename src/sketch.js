...
function draw() {
  if (!pause) {
    ...
    for (let i = 0; i < Math.floor(UPDATE_RATE / FRAME_RATE); i++) {
      ...
      robot.heading = p5.Vector.sub(robot.units[1].pos, robot.units[0].pos).heading();
      ...
      if (keyIsDown(RIGHT_ARROW)) { userVel.x += xyVisualVelocity / UPDATE_RATE; }
      if (keyIsDown(LEFT_ARROW)) { userVel.x -= xyVisualVelocity / UPDATE_RATE; }
      if (keyIsDown(DOWN_ARROW)) { userVel.y += xyVisualVelocity / UPDATE_RATE; }
      if (keyIsDown(UP_ARROW)) { userVel.y -= xyVisualVelocity / UPDATE_RATE; }

      if (keyIsDown(68)) { angleVel = angleVisualVelocity / UPDATE_RATE; }
      else if (keyIsDown(65)) { angleVel = -angleVisualVelocity / UPDATE_RATE; }
